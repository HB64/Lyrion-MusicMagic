package Plugins::MusicMagicCE::Common;
...
$prefs->setChange(
	sub {
		...
		if ($newval) {
			Plugins::MusicMagicCE::Plugin->initPlugin();
		}

		Slim::Music::Import->useImporter('Plugins::MusicMagicCE::Plugin', $_[1]);
...
$prefs->setChange(
	sub {
		Slim::Utils::Timers::killTimers(undef, \&Plugins::MusicMagicCE::Plugin::checker);

		my $interval = $prefs->get('scan_interval') || 3600;

		main::INFOLOG && $log->info("re-setting scaninterval to $interval seconds.");

		Slim::Utils::Timers::setTimer(undef, Time::HiRes::time() + 120, \&Plugins::MusicMagicCE::Plugin::checker);
	},
'scan_interval');
