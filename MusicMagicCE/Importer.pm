package Plugins::MusicMagicCE::Importer;
...
sub initPlugin {
	...
	my $initialized = get( Plugins::MusicMagicCE::Common::getBaseUrl() . '/version', 5 );
...
sub doneScanning {
	...
	my $lastDate = get(Plugins::MusicMagicCE::Common::getBaseUrl() . '/cacheid?contents');
...
sub exportSongs {
	...
	if ($fullRescan == 1 || $prefs->get('musicip') == 1) {
		...
		my $count = get(Plugins::MusicMagicCE::Common::getBaseUrl() . '/getSongCount');
		if ($count) {
			# convert to integer
			chomp($count);
			$count += 0;
		...
		if (Slim::Utils::Versions->compareVersions($MMMVersion, '1.5') >= 0) {
			...
			my $MMMDataURL  = Plugins::MusicMagicCE::Common::getBaseUrl() . '/songs?extended';
			...
			for (my $scan = 0; $scan <= $count; $scan++) {
				my $content = get(Plugins::MusicMagicCE::Common::getBaseUrl() . "/getSong?index=$scan");
		...
		for my $track (@notMixableTracks) {
			...
			my $pathEnc = Plugins::MusicMagicCE::Common::escape($path);

			# Set musicmagic_mixable on $track object and call $track->update to actually store it.
			my $result = get(Plugins::MusicMagicCE::Common::getBaseUrl() . "/status?song=$pathEnc");
...
sub processSong {
	...
	for my $key (@keys) {
		...
		$songInfo{$key} = Plugins::MusicMagicCE::Common::decode($songInfo{$key});
	}
	...
	if (main::ISWINDOWS) {
		...
		$songInfo{'file'} = Plugins::MusicMagicCE::Common::translatePath($songInfo{'file'});
	}
...
sub exportPlaylists {
	...
	my @playlists = split(/\n/, get(Plugins::MusicMagicCE::Common::getBaseUrl() . '/playlists'));
	...
	for (my $i = 0; $i <= scalar @playlists; $i++) {
		...
		my $listname = Plugins::MusicMagicCE::Common::decode($playlists[$i]);

		my $playlist = get(Plugins::MusicMagicCE::Common::getBaseUrl() . "/getPlaylist?index=$i") || next;
		my @songs    = split(/\n/, $playlist);
...
sub exportDuplicates {
	...
	my @songs = split(/\n/, get(Plugins::MusicMagicCE::Common::getBaseUrl() . '/duplicates'));
...
sub _updatePlaylist {
	...
	my %attributes = ();
	my $url        = 'musicipplaylist:' . Plugins::MusicMagicCE::Common::escape($name);
