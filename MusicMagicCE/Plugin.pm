package Plugins::MusicMagicCE::Plugin;
...
if ( main::WEBUI ) {
	require Plugins::MusicMagicCE::Settings;
	require Plugins::MusicMagicCE::ClientSettings;
}

use Plugins::MusicMagicCE::Common;
use Plugins::MusicMagicCE::PlayerSettings;
...
sub shutdownPlugin {
	...
	# forced rescan
	Slim::Music::Import->useImporter('Plugins::MusicMagicCE::Plugin', 0);
}
...
sub initPlugin {
	...
	Plugins::MusicMagicCE::Common::checkDefaults();

	if ( main::WEBUI ) {
		Plugins::MusicMagicCE::Settings->new;
	}
	...
	main::INFOLOG && $log->info("Testing for API on " . Plugins::MusicMagicCE::Common::getBaseUrl());
	...
	if ($response->is_error) {
		...
		$log->error("Can't connect to " . Plugins::MusicMagicCE::Common::getBaseUrl() . " - MusicIP disabled.");
		...
		Plugins::MusicMagicCE::PlayerSettings::init();
		...
		Plugins::MusicMagicCE::Common->grabFilters();

		if ( main::WEBUI ) {
			Plugins::MusicMagicCE::ClientSettings->new;
		}
		...
		Slim::Player::ProtocolHandlers->registerHandler(
			mood => 'Plugins::MusicMagicCE::ProtocolHandler'
		);
		...
		if (scalar @{grabMoods()}) {
			...
			if ( main::WEBUI ) {
				Slim::Web::Pages->addPageLinks("browse", {
					'MUSICMAGIC_MOODS' => "plugins/MusicMagicCE/musicmagic_moods.html"
				});
			}
			...
			Slim::Web::Pages->addPageLinks("icons", {
				'MUSICMAGIC_MOODS' => "plugins/MusicMagicCE/html/images/icon.png"
			});
			...
			Slim::Control::Jive::registerPluginMenu([{
				...
				window         => {
					'icon-id'  => 'plugins/MusicMagicCE/html/images/icon.png',
					titleStyle => 'moods'
				},
			}]);
		}
...
sub isMusicLibraryFileChanged {
	...
	$http->get( Plugins::MusicMagicCE::Common::getBaseUrl() . '/cacheid?contents' );
}
...
sub _cacheidOK {
	...
	$http->get( Plugins::MusicMagicCE::Common::getBaseUrl() . '/getStatus' );
}
...
sub getMix {
	...
	if ($filter) {
		...
		$args{'filter'} = Plugins::MusicMagicCE::Common::escape($filter);
	}
	...
	my $mixArgs = join('&', map {
		...
		$validMixTypes{$for} . '=' . Plugins::MusicMagicCE::Common::escape($id);
	} @ids);

	main::DEBUGLOG && $log->debug("Request " . Plugins::MusicMagicCE::Common::getBaseUrl() . "/mix?$mixArgs\&$argString");
	...
	for (my $j = 0; $j < $count; $j++) {
		...
		$songs[$j] = Plugins::MusicMagicCE::Common::translatePath($songs[$j]);
...
sub musicmagic_moods {
	...
	return Slim::Web::HTTP::filltemplatefile("plugins/MusicMagicCE/musicmagic_moods.html", $params);
}
...
sub musicmagic_mix {
	...
	$params->{'pwd_list'} .= ${Slim::Web::HTTP::filltemplatefile("plugins/MusicMagicCE/musicmagic_pwdlist.html", $params)};
	...
	return Slim::Web::HTTP::filltemplatefile("plugins/MusicMagicCE/musicmagic_mix.html", $params);
}
...
sub _objectInfoHandler {
	...
	if ( $mixable ) {
		return {
			...
			web  => {
				url   => 'plugins/MusicMagicCE/musicmagic_mix.html?' . $special->{urlKey} . '=' . $obj->id,
			},
		};
	}
...
sub _syncHTTPRequest {
	...
	return $http->get(Plugins::MusicMagicCE::Common::getBaseUrl() . $url);
}
...
sub checkSingleTrack {
	...
	require Plugins::MusicMagicCE::Importer;
	Plugins::MusicMagicCE::Importer->initPlugin();

	my $path   = Slim::Utils::Misc::pathFromFileURL($url);
	my $apiurl = Plugins::MusicMagicCE::Common::getBaseUrl() . '/getSong?file=' . uri_escape_utf8($path);
	...
	my $http = Slim::Networking::SimpleAsyncHTTP->new(
		sub {
			...
			if ( $http->content =~ /active\s+yes/ ) {
				...
				Plugins::MusicMagicCE::Importer->setSongMixable($track);
			}
			else {
