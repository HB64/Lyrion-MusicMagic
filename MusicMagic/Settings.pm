...
sub handler {
	...
	if ( !$params->{'saveSettings'} && !$params->{'filters'} ) {
		...
		Plugins::MusicMagic::Common::grabFilters($class, $client, $params, $callback, @args);
	...
	$params->{'filters'}    = Plugins::MusicMagic::Common->getFilterList();
	$params->{'genre_list'} = Plugins::MusicMagic::Common::getGenreList();
