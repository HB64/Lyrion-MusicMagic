...
sub initPlugin {
	...
	main::INFOLOG && $log->info("Testing for API on " . Plugins::MusicMagic::Common::getBaseUrl());

	my $initialized = get( Plugins::MusicMagic::Common::getBaseUrl() . '/version', 5 );
...
sub doneScanning {
	...
	my $lastDate = get(Plugins::MusicMagic::Common::getBaseUrl() . '/cacheid?contents');
...
sub exportSongs {
	...
	if ($fullRescan == 1 || $prefs->get('musicip') == 1) {
		...
		my $count = get(Plugins::MusicMagic::Common::getBaseUrl() . '/getSongCount');
		if ($count) {
			# convert to integer
			chomp($count);
			$count += 0;
		...
		if (Slim::Utils::Versions->compareVersions($MMMVersion, '1.5') >= 0) {
			...
			my $MMMDataURL  = Plugins::MusicMagic::Common::getBaseUrl() . '/songs?extended';
			...
			for (my $scan = 0; $scan <= $count; $scan++) {
				my $content = get(Plugins::MusicMagic::Common::getBaseUrl() . "/getSong?index=$scan");
		...
		for my $track (@notMixableTracks) {
			...
			my $pathEnc = Plugins::MusicMagic::Common::escape($path);

			# Set musicmagic_mixable on $track object and call $track->update to actually store it.
			my $result = get(Plugins::MusicMagic::Common::getBaseUrl() . "/status?song=$pathEnc");
...
sub processSong {
	...
	for my $key (@keys) {
		...
		$songInfo{$key} = Plugins::MusicMagic::Common::decode($songInfo{$key});
	}
	...
	if (main::ISWINDOWS) {
		...
		$songInfo{'file'} = Plugins::MusicMagic::Common::translatePath($songInfo{'file'});
	}
...
sub exportPlaylists {
	...
	my @playlists = split(/\n/, get(Plugins::MusicMagic::Common::getBaseUrl() . '/playlists'));
	...
	for (my $i = 0; $i <= scalar @playlists; $i++) {
		...
		my $listname = Plugins::MusicMagic::Common::decode($playlists[$i]);

		my $playlist = get(Plugins::MusicMagic::Common::getBaseUrl() . "/getPlaylist?index=$i") || next;
		my @songs    = split(/\n/, $playlist);
...
sub exportDuplicates {
	...
	my @songs = split(/\n/, get(Plugins::MusicMagic::Common::getBaseUrl() . '/duplicates'));
...
sub _updatePlaylist {
	...
	my %attributes = ();
	my $url        = 'musicipplaylist:' . Plugins::MusicMagic::Common::escape($name);
