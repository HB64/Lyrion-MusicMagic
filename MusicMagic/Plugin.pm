...
if ( main::WEBUI ) {
	require Plugins::MusicMagic::Settings;
	require Plugins::MusicMagic::ClientSettings;
}
...
sub initPlugin {
	...
	if ($response->is_error) {
		...
		$log->error("Can't connect to " . Plugins::MusicMagic::Common::getBaseUrl() . " - MusicIP disabled.");
		...
		Plugins::MusicMagic::Common->grabFilters();

		if ( main::WEBUI ) {
			Plugins::MusicMagic::ClientSettings->new;
		}
		...
		Slim::Player::ProtocolHandlers->registerHandler(
			mood => 'Plugins::MusicMagic::ProtocolHandler'
		);
...
sub isMusicLibraryFileChanged {
	...
	$http->get( Plugins::MusicMagic::Common::getBaseUrl() . '/cacheid?contents' );
}
...
sub _cacheidOK {
	...
	$http->get( Plugins::MusicMagic::Common::getBaseUrl() . '/getStatus' );
}
...
sub getMix {
	...
	if ($filter) {
		...
		$args{'filter'} = Plugins::MusicMagic::Common::escape($filter);
	}
	...
	main::DEBUGLOG && $log->debug("Request " . Plugins::MusicMagic::Common::getBaseUrl() . "/mix?$mixArgs\&$argString");
	...
	for (my $j = 0; $j < $count; $j++) {
		...
		$songs[$j] = Plugins::MusicMagic::Common::translatePath($songs[$j]);
...
sub _syncHTTPRequest {
	...
	return $http->get(Plugins::MusicMagic::Common::getBaseUrl() . $url);
}
...
sub checkSingleTrack {
	...
	require Plugins::MusicMagic::Importer;
	Plugins::MusicMagic::Importer->initPlugin();

	my $path   = Slim::Utils::Misc::pathFromFileURL($url);
	my $apiurl = Plugins::MusicMagic::Common::getBaseUrl() . '/getSong?file=' . uri_escape_utf8($path);
	...
	my $http = Slim::Networking::SimpleAsyncHTTP->new(
		sub {
			...
			if ( $http->content =~ /active\s+yes/ ) {
				...
				Plugins::MusicMagic::Importer->setSongMixable($track);
			}
			else {
