...
$prefs->setChange(
	sub {
		...
		if ($newval) {
			Plugins::MusicMagic::Plugin->initPlugin();
		}

		Slim::Music::Import->useImporter('Plugins::MusicMagic::Plugin', $_[1]);
...
$prefs->setChange(
	sub {
		Slim::Utils::Timers::killTimers(undef, \&Plugins::MusicMagic::Plugin::checker);

		my $interval = $prefs->get('scan_interval') || 3600;

		main::INFOLOG && $log->info("re-setting scaninterval to $interval seconds.");

		Slim::Utils::Timers::setTimer(undef, Time::HiRes::time() + 120, \&Plugins::MusicMagic::Plugin::checker);
	},
'scan_interval');
